...
- [x] ISSUE-057: [Architect] Implement CSV Input Data Support [CLOSED]
...
- [x] ISSUE-059: [Architect] Design and Implement Advanced CSV Handling (Multi-axis Synthesis & Intelligent Selection) [CLOSED]
    *   **[Architect] Subtask 1: Define multi-axis synthesis logic and column inference strategy.** [DONE]
    *   **[Developer] Subtask 2: Extend `csv_parser.py` to support multiple column reading and vector magnitude calculation.** [DONE]
    *   **[Developer] Subtask 3: Enhance Streamlit UI with multi-column selection, synthesis options, and data preview statistics.** [DONE]
    *   **[QA/Expert] Subtask 4: Verify with real-world 3-axis CSV data and ensure audit log consistency.** [DONE]
- [x] ISSUE-060: [QA/Expert] Enhance CSV Parsing Robustness and Unit Test Coverage [CLOSED]
    *   **[Developer] Subtask 1: Extract column inference logic to `csv_parser.py` for better testability.** [DONE]
...
- [x] ISSUE-067: [Developer] 解析プリセットの永続化ロジックとStreamlit UIの実装（保存処理の未実装バグおよび複数読み込みバグを修正） [CLOSED]
...
- [x] ISSUE-074: [Developer] アップロード済みファイルからの一括単位空間構築機能の実装 (パワー平均の導入とバグ修正を完了) [CLOSED]
...
- [x] ISSUE-078: [Architect] サマリー解析強化の設計 (周波数ヒートマップ & PCA可視化) [CLOSED]
- [x] ISSUE-079: [Developer] 全データの周波数ヒートマップ表示機能の実装 [CLOSED]
- [x] ISSUE-080: [Developer] 特徴量PCAによる類似度可視化機能の実装 [CLOSED]
- [x] ISSUE-081: [QA/Expert] 物理的妥当性と計算パフォーマンスの検証 [CLOSED]
- [x] ISSUE-082: [Architect] 傾向分析高度化の設計 (バイプロット & 箱ひげ図 & トレンド) [CLOSED]
- [x] ISSUE-083: [Developer] PCAバイプロット（因子負荷量）の可視化実装 [CLOSED]
...
- [x] ISSUE-099: [Architect] グループ別トレンド解析の設計 (サブプロット & 移動平均) [CLOSED]
- [x] ISSUE-100: [Developer] グループ別サブプロットと移動平均推移の実装 [CLOSED]
- [x] ISSUE-101: [QA/Expert] スケールの一貫性と描画負荷の検証 [CLOSED]
- [x] ISSUE-102: [Architect] 動적トレンド解析の設計 (指標選択 & 軸管理) [CLOSED]
- [x] ISSUE-103: [Developer] トレンド指標選択機能の実装とRMSデフォルト化 [CLOSED]
- [x] ISSUE-104: [QA/Expert] 単位・スケールの動的整合性と描画検証 [CLOSED]
- [ ] ISSUE-105: [Architect] セグメント・トレンドのUI/UX設計 (X軸ソート & 区画化) [OPEN]
- [ ] ISSUE-106: [Developer] X軸グループソート機能とセグメント表示の実装 [OPEN]
- [x] ISSUE-107: [QA/Expert] データの連続性と分離の正確性検証 [CLOSED]
- [x] ISSUE-108: [Architect] コンテキスト入力項目（目的/対象/環境）の設計 [CLOSED]
- [x] ISSUE-109: [Developer] コンテキスト入力UIと構造化データ変換エンジンの実装 [CLOSED]
- [x] ISSUE-110: [Developer] テーマ別高度プロンプト・ジェネレーターの実装 (Role & Constraint) [CLOSED]
- [x] ISSUE-111: [QA/Expert] AI回答の安定性を高める「物理/診断基準プロンプト」の検証 [CLOSED]
- [x] ISSUE-112: [Architect] 統計要約ロジックとISO評価基準のマッピング設計 [CLOSED]
- [x] ISSUE-113: [Developer] 自動匿名化 ＆ 統計ナラティブ生成エンジンの実装 [CLOSED]
- [x] ISSUE-114: [Developer] ISO 10816 クラス選択UIとプロンプト統合の実装 [CLOSED]
- [x] ISSUE-115: [QA/Expert] 統計計算の正確性と匿名化強度の検証 [CLOSED]
- [x] ISSUE-116: [Architect] 物理制約卓越ピーク抽出と診断ナラティブの設計 [CLOSED]
- [x] ISSUE-117: [Developer] 卓越周波数リストの自動生成とプロンプト統合の実装 [CLOSED]
- [x] ISSUE-118: [Developer] グループ間比較（ギャップ分析）プロンプトの高度化 [CLOSED]
- [x] ISSUE-119: [QA/Expert] 物理推論ヒントの正確性とAI回答品質の検証 [CLOSED]
- [x] ISSUE-120: [Architect] エビデンス・パッケージ（ZIP）の構造とAIファイル説明の設計 [CLOSED]
- [x] ISSUE-121: [Developer] メモリ内ZIPアーカイブ生成エンジンの実装 [CLOSED]
- [x] ISSUE-122: [Developer] 統合レポートダウンロードUIの実装 [CLOSED]
